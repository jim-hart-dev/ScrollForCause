# Plan: Deploy ScrollForCause Landing Page

## Context

ScrollForCause has a single-file landing page (`swipeforcause-landing.html`) that needs to go live. The page has two email waitlist forms (volunteer + nonprofit) that currently only `console.log` submissions. We need to:
...
The user has asked for a copy at C:\Code\ScrollForCause\PLAN.md
